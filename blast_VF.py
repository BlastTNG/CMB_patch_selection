...
modified by V.Fanfani and F.Nati
"""
...
#Import all maps from PySM
nside=128  ## THE RESOLUTION OF THE MAP (the same of fits files from PySM)  for 30 arcmin resolution

f500='./PySM_public-master/Output/Output/128_MJySr_cmb_spinn_synch_therm_600p0_128.fits'
f350='./PySM_public-master/Output/Output/128_MJySr_cmb_spinn_synch_therm_600p0_128.fits'
f250='./PySM_public-master/Output/Output/128_MJySr_cmb_spinn_synch_therm_600p0_128.fits'

"""
nside=2048  ## THE RESOLUTION OF THE MAP (the same of fits files from PySM)  for 1.7 arcmin resolution
...
#SPIDER visibility intersection
x_spider_intersection=np.array([910,776,1050,1253,1306,1093,910])*(360.0/4000)
y_spider_intersection=np.array([800,644,339,339,384,800,800])*(180.0/2000)-90
...
for v in range(0,3):
	...
	connect_the_dots(x20,y20,'white') #plot the 20hr/day contoury
	connect_the_dots(x_spider_intersection,y_spider_intersection,'black') #plot the contoury of spider intersection
	...
	hp.cartview(sim_PF[v],title=channel[v]+' $\mu$m FTF1 Polarization Fraction side '+str(patch_size)+'deg',lonra=[ra0_effect-w,ra0_effect+w], latra=[dec0_effect-w,dec0_effect+w],min=0,max=0.2,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
	...
	hp.cartview(sim_PF[v],title=channel[v]+' $\mu$m FTF2 Polarization Fraction side 5deg',lonra=[ra0_comparison-w_comparison,ra0_comparison+w_comparison], latra=[dec0_comparison-w_comparison,dec0_comparison+w_comparison],min=0,max=0.2,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
	...
	hp.cartview(sim_SNR[v],title=channel[v]+' $\mu$m FTF1 Polarized SNR for '+str(tmap)+'h on '+str(patch_size)+'x'+str(patch_size)+'deg Patch',lonra=[ra0_effect-w,ra0_effect+w], latra=[dec0_effect-w,dec0_effect+w],min=0,max=10,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
	...
	hp.cartview(sim_SNR[v],title=channel[v]+' $\mu$m FTF2 Polarized SNR for '+str(tmap)+'h on '+str(patch_size)+'x'+str(patch_size)+'deg Patch size 5deg',lonra=[ra0_comparison-w_comparison,ra0_comparison+w_comparison], latra=[dec0_comparison-w_comparison,dec0_comparison+w_comparison],min=0,max=10,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
	...
	for i in range(70,243,patch_size):  #longitude range of the 10 h visibility area with the step given by the pixel size
		for j in range(18,61,patch_size):  #latitude range of the 10 h visibility area with the step given by the pixel size 
			...
			coord_eq = SkyCoord(i, -j, frame='icrs', unit='deg')
			coord_gal = coord_eq.galactic
			lat = coord_gal.b

			# limit in latitude (degrees) of proximity to the GALACTIV PLANE
			if abs(lat.value) <=10:
				sim_Q[v][ipix_patch] = -100.
				sim_U[v][ipix_patch] = -100.
				sim_Pol[v][ipix_patch] = -100.
				sim_PF[v][ipix_patch] = -100.
				sim_SNR[v][ipix_patch] = -100.
				...
				#sim_SNR[v][ipix_patch] = pol_contrast/Spix[v]
					...
					#print('PolFrac Quality Factor ',sim_PF[v][ipix_patch].mean())
					#print('PolSNR Quality Factor ',sim_SNR[v][ipix_patch].mean())
					#print('PolSNR without contrast Quality Factor ',sim_SNR[v][ipix_patch].mean())
	...
	#for i in range(0, len(ra_good)-1):
		#print(ra_good[i], dec_good[i])
	...
	hp.cartview(sim_PF[v],title=channel[v]+' $\mu$m FTF1 Polarization Fraction Quality Factor '+str(patch_size)+'x'+str(patch_size)+'deg Patch',lonra=[ra0_effect-w,ra0_effect+w], latra=[dec0_effect-w,dec0_effect+w],min=0,max=0.2,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
 	fig = plt.gcf()
	ax = plt.gca()
	image = ax.get_images()[0]
	cb = fig.colorbar(image, ax=ax)
	...
	hp.cartview(sim_PF[v],title=channel[v]+' $\mu$m FTF2 Polarization Fraction Quality Factor '+str(patch_size)+'x'+str(patch_size)+'deg Patch',lonra=[ra0_comparison-w_comparison,ra0_comparison+w_comparison], latra=[dec0_comparison-w_comparison,dec0_comparison+w_comparison],min=0,max=0.2,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
 	fig = plt.gcf()
	ax = plt.gca()
	image = ax.get_images()[0]
	cb = fig.colorbar(image, ax=ax)
	...
	hp.cartview(sim_SNR[v],title=channel[v]+' $\mu$m FTF1 Polarized SNR Quality Factor for '+str(tmap)+'h on'+str(patch_size)+'x'+str(patch_size)+'deg Patch',lonra=[ra0_effect-w,ra0_effect+w], latra=[dec0_effect-w,dec0_effect+w],min=0,max=10,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
 	fig = plt.gcf()
	ax = plt.gca()
	image = ax.get_images()[0]
	cb = fig.colorbar(image, ax=ax)
	...
	hp.cartview(sim_SNR[v],title=channel[v]+' $\mu$m FTF2 Polarized SNR Quality Factor for '+str(patch_size)+'x'+str(patch_size)+'deg Patch',lonra=[ra0_comparison-w_comparison,ra0_comparison+w_comparison], latra=[dec0_comparison-w_comparison,dec0_comparison+w_comparison],min=0,max=10,cmap='seismic',cbar=True,coord=['G','C'],xsize=nside)
 	fig = plt.gcf()
	ax = plt.gca()
	image = ax.get_images()[0]
	cb = fig.colorbar(image, ax=ax)
